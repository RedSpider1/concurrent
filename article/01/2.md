...
注意，`run`方法里面的内容是我们要在这个线程里面执行的代码。`run`方法不需要我们在程序里显示调用，而是**留给JVM调用**的。
...
> 这里的run()方法是提供给虚拟机调用的。我们在程序里面调用了start()方法后，虚拟机会为我们创建一个线程，然后会在这个线程在第一次得到时间片时调用run()方法。
>
...
`Thread`类里面有一个`public`的内部枚举类，用于标识Java里线程的状态。其结构如下：
...
我们会在第四章来详细解释线程的状态、它们之间的转化关系及常用的方法。

### 2.1.5 Thread类的几个常用方法
...
`Callable`与`Runnable`类似，同样是只有一个抽象方法的函数式接口。不同的是，`Callable`提供的方法是**有返回值**的，而且支持**泛型**。另外一个区别是它可以显示地抛出一个异常。
