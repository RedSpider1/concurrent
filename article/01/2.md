...
>
> 注意不可多次调用start()方法。在第一次调用start()方法后，该线程对象的started变量值为true，表示已被启动过了，再次调用start()方法会抛出异常。
...
在很多高并发的环境下，有可能Callable和FutureTask会创建多次。`FutureTask`能够在高并发环境下**确保任务只执行一次**。这块有兴趣的同学可以参看`FutureTask`源码。
