...
ThreadGroup和Thread的关系就如同他们的字面意思一样简单粗暴，每个Thread必然存在于一个ThreadGroup中，Thread不能独立于ThreadGroup存在。**默认的初始线程名字是main，默认的线程组的名字也是main**。
...
public class Demo {
    public static void main(String[] args) {
        ...
        testThread.start();
        System.out.println("初始线程名字：" + Thread.currentThread().getName());
    }
}
...
Java中的优先级来说不是特别的可靠，**Java程序中对线程所设置的优先级只是给操作系统一个建议，操作系统不一定会采纳。而真正的调用顺序，是由系统的CPU调度决定的**。
...
public class Demo {
    public static class T1 extends Thread {
        ...
        public void run() {
            super.run();
            System.out.println(String.format("当前线程是：%s，优先级：%d",
                    Thread.currentThread().getName(),
...
// 某次输出：
当前线程是：Thread-17，优先级：9
当前线程是：Thread-1，优先级：1
当前线程是：Thread-13，优先级：7
当前线程是：Thread-11，优先级：6
当前线程是：Thread-15，优先级：8
当前线程是：Thread-7，优先级：4
当前线程是：Thread-9，优先级：5
当前线程是：Thread-3，优先级：2
当前线程是：Thread-5，优先级：3
```

从结果可以看到，尽管我们指定了不同的线程优先级，优先级比较高的线程会有更大的几率先被执行，但运行顺序并不是完全根据我们指定的优先级决定的。
...
> 如果某线程是守护线程，那如果主线程结束，这个守护线程也会自动结束。
>
> 应用场景是：当主线程结束时，结束其余的子线程（守护线程）自动关闭，就免去了还要继续关闭子线程的麻烦。
>
