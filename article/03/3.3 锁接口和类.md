...
事实上不只是AQS和AQLS会继承这个类，其它很多地方比如线程池框架`ThreadPoolExecutor`类里面的工作线程内部类也继承了AQS：
...
之前我们提到了每个对象都可以用继承自`Object`的**wait/notify**方法来实现**“等待/通知”机制**。而Condition接口也提供了类似Object监视器的方法，通过与**Lock**配合来实现等待/通知模式。
...
Condition和Object的wait/notify基本相似。其中，Condition的await方法对应的是Object的wait方法，而Condition的**signal/signalAll**方法则对应Object的notify/notifyAll()。不同的是，Condition还有如下两个方法来增强等待/通知机制：
