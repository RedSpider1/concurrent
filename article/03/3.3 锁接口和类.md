...
Condition和Object的wait/notify基本相似。其中，Condition的await方法对应的是Object的wait方法，而Condition的signal/signalAll方法则对应Object的notify/notifyAll()。不同的是，Condition还有如下两个方法来增强等待/通知机制：

- void awaitUninterruptibly();
- boolean awaitUntil(Date deadline) throws InterruptedException;

第一个方法可以在等待忽略线程中断，不会抛出中断异常。而第二个方法可以使线程等待到某个特定的时刻。
...
这个类也是一个非抽象类，它是ReadWriteLock接口的JDK默认实现。它与ReentrantLock的功能类似，同样是可重入的，支持非公平说和公平锁。不同的是，它还支持”读写锁“。
