...
public final ForkJoinTask<V> fork() {
    ...
    if ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread)
        ...
        // 没有显示创建ForkJoinPool的时候走这里，提交任务到默认提供的common线程池中
...
我们在之前介绍过说Thread.join()会使线程阻塞，而ForkJoinPool.join()会使免于阻塞，下面是ForkJoinPool.join()的流程图：
![join流程图](../img/join()流程图.png)
...
此外，两个子类都有执行主要计算的方法compute()，当然，RecursiveAction无返回值，RecursiveTask的compute()有具体的返回值。
...
public class ForkJoinPool extends AbstractExecutorService {
    ...
    // 其他构造方法都是源自于此方法
    private ForkJoinPool(int parallelism, // 并行度,默认调用java.lang.Runtime.availableProcessors() 方法返回可用处理器的Java虚拟机的数量
...
需要注意的是，上述计算时间复杂度为`$O(2^n)$`	，随着n的增长计算效率会越来越低。此外，也并不是所有的任务都适合Fork/Join框架，比如上面的例子任务划分过于细小反而体现不出效率，大家可以下去试试用普通的递归来求f(n)的值，看看是不是要比使用Fork/Join快。

另外，Java8流操作的相关方法也用到了Fork/Join（比如sort()方法），大家如果有兴趣可以去看一看源码。
