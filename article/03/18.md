...
public final ForkJoinTask<V> fork() {
    ...
    if ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread)
        ...
        // 没有显式创建ForkJoinPool的时候走这里，提交任务到默认提供的common线程池中
...
需要注意的是，上述计算时间复杂度为`$O(2^n)$`，随着n的增长计算效率会越来越低，这也是上面的例子中n不敢取太大的原因。此外，也并不是所有的任务都适合Fork/Join框架，比如上面的例子任务划分过于细小反而体现不出效率，下面我们试试用普通的递归来求f(n)的值，看看是不是要比使用Fork/Join快：
